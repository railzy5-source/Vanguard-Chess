...
import { classifyMove } from './analysis/moveClassifier.js';
...
class VanguardChessApp {
  constructor() {
    ...
    this.playerAccuracyCalc = new AccuracyCalculator();
  ...
  async handlePlayerMove(move) {
    ...
    try {
      const playerColorLabel = move.color === 'w' ? 'white' : 'black';

      // Re-evaluates the exact resulting position (not just a top-3 guess),
      // so classification is accurate even for moves outside the engine's
      // top candidates.
      const evalResult = await this.engine.evaluateMove({
        fenBefore,
        playedMove: uci,
        playerColor: playerColorLabel
      });

      if (evalResult) {
        const candidates = evalResult.multiPV || [];
        bestMoveSan = candidates[0]?.san || '';
        playedScore = evalResult.playedEval;
        scoreFromWhite = move.color === 'w' ? playedScore : -playedScore;

        const analysis = classifyMove({
          playedMove: uci,
          bestMove: evalResult.bestMove,
          evalBefore: evalResult.bestEval,
          evalAfter: evalResult.playedEval,
          bestEval: evalResult.bestEval,
          fenBefore,
          fenAfter: evalResult.fenAfter,
          playerColor: playerColorLabel,
        ...
        // Real chess.com-style accuracy (win-probability based), tracked
        // live so the Game Review tab has genuine data the moment it's opened.
        const moveAcc = this.playerAccuracyCalc.addMove(evalResult.bestEval, evalResult.playedEval);
    ...
    this.coach.reactToMove({
      ...
      fenBefore: this.moveHistory.length >= 2 ? this.moveHistory[this.moveHistory.length - 2].fen : new Chess().fen(),
