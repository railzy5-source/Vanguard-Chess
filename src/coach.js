...
import { classifyMove, getAccuracy, getScaleForElo } from './moveClassifier.js';
...
class VanguardChessApp {
  ...
  async handlePlayerMove(move) {
    ...
    try {
      ...
      if (candidates && candidates.length > 0) {
        ...
        // Classify using unified function - player moves judged at master level (scale=1.0)
        classification = classifyMove(uci, bestUci, topScore, playedScore, 1.0);

        // Accuracy tracking
        const moveAcc = getAccuracy(diff, 1.0);
  ...
  async triggerAiMove() {
    ...
    try {
      ...
      if (bestMoveUci && this.gameActive) {
        ...
        if (moveRes) {
          ...
          if (result.multiPV && result.multiPV.length > 0) {
            ...
            // FIX #4: Handle when played move not in top-3 analysis
            // If move wasn't in multiPV (e.g., random blunder), estimate it as much worse
            if (!playedAiMove) {
              // Random blunder or very weak move - estimate 5+ pawns worse
              aiScore = bestAiMove.score - 500;
            } else {
              aiScore = playedAiMove.score;
            }
            
            const topScore = bestAiMove.score;
            const aiDiff = Math.max(0, topScore - aiScore);
            
            // Get scale factor for AI's current ELO (FIX #1: Now correctly uses currentElo)
            const aiElo = this.engine.currentElo || 1500;
            const scale = getScaleForElo(aiElo);
            
            // Classify using unified function (FIX #14: Use classifyMove)
            aiClassification = classifyMove(bestMoveUci, bestAiMove.moveUci, topScore, aiScore, scale);
            
            console.log(`🤖 AI Move: ${moveRes.san} | Diff: ${aiDiff}cp | ELO: ${aiElo} | Scale: ${scale}x | Class: ${aiClassification}`);
          }
      ...
      // Graceful recovery: try fallback if engine fails
      this.coach.speak('Engine encountered an issue. Let me think again...', 'surprised');
